...
MeshTransform& Drawer::getMeshTransform(){
    return _mesh_transform;
}

const MeshTransform& Drawer::getMeshTransform() const {
    return _mesh_transform;
}
...
const sptr<gl::Buffer> Drawer::_getUniformBlockBuffer(){
    ...
    } else if constexpr (U == DrawerUniformBlock::MeshTransform){
        return _mesh_transform.buffer();
    } else {
