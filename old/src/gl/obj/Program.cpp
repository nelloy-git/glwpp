...
bool Program::setUniform_1I(const Val<const Int>& unif_loc, const Val<const Int>& data,
                            const Val<const Sizei>& count, const Val<const SrcLoc>& src_loc){
    if (!isContextThread()){
        return executeMethodInContext(&Program::setUniform_1I, unif_loc, data, count, src_loc);
    }

    glProgramUniform1iv(id(), unif_loc, count, data);
    debug(src_loc);
    return true;
}
