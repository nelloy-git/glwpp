...
class GlIndirect : public Gl {
public:
    GlIndirect(Context& ctx, const std::function<void(const SrcLoc&)>& debug);
    ~GlIndirect();
    ...
    // EXPORT void DebugMessageCallback(const GLDEBUGPROC& callback, const GLdata& userParam, SrcLoc src_loc = SrcLoc{});
    // EXPORT void GetUniformIndices(const GLuint& program, const GLsizei& uniformCount, const GLchar& *const* uniformNames, const GLuintArr& uniformIndices, SrcLoc src_loc = SrcLoc{});
    // EXPORT void ShaderSource(const GLuint& shader, const GLsizei& count, const GLchar& *const* string, const GLintArr& length, SrcLoc src_loc = SrcLoc{});
    // EXPORT void TransformFeedbackVaryings(const GLuint& program, const GLsizei& count, const GLchar& *const* varyings, const GLenum& bufferMode, SrcLoc src_loc = SrcLoc{});

private:
    Context* _ctx;

    template<auto F, typename ... Args>
    inline auto _callGL(Args... args);
