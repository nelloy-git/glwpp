...
bool Buffer::data(const Val<const SizeiPtr>& size, const Val<const void>& data, const Val<const BufferUsage>& usage,
    ...
    return executeInContext(src_loc, glNamedBufferData, id(), size, data, usage.cast_reinterpret<const Enum>());
}
...
bool Buffer::getMapAccess(const Val<BufferMapAccess>& dst, const Val<const SrcLoc>& src_loc) const {
    return executeInContext(src_loc, glGetBufferParameteriv, id(), GL_BUFFER_ACCESS, dst.cast_reinterpret<Int>());
}

bool Buffer::getMapRangeAccess(const Val<BitField>& dst, const Val<const SrcLoc>& src_loc) const {
    return executeInContext(src_loc, glGetBufferParameteriv, id(), GL_BUFFER_ACCESS_FLAGS, dst.cast_reinterpret<Int>());
}

bool Buffer::isImmutable(const Val<bool>& dst, const Val<const SrcLoc>& src_loc) const {
    return executeInContext(src_loc, glGetBufferParameteriv, id(), GL_BUFFER_IMMUTABLE_STORAGE, dst.cast_reinterpret<Int>());
}

bool Buffer::isMapped(const Val<bool>& dst, const Val<const SrcLoc>& src_loc) const {
    return executeInContext(src_loc, glGetBufferParameteriv, id(), GL_BUFFER_MAPPED, dst.cast_reinterpret<Int>());
}

bool Buffer::getMapLength(const Val<Int64>& dst, const Val<const SrcLoc>& src_loc) const {
    return executeInContext(src_loc, glGetBufferParameteri64v, id(), GL_BUFFER_MAP_LENGTH, dst);
}

bool Buffer::getMapOffset(const Val<Int64>& dst, const Val<const SrcLoc>& src_loc) const {
    return executeInContext(src_loc, glGetBufferParameteri64v, id(), GL_BUFFER_MAP_OFFSET, dst);
}

bool Buffer::getSize(const Val<Int>& dst, const Val<const SrcLoc>& src_loc) const {
    return executeInContext(src_loc, glGetBufferParameteriv, id(), GL_BUFFER_SIZE, dst.cast_reinterpret<Int>());
}

bool Buffer::getStorageFlags(const Val<BitField>& dst, const Val<const SrcLoc>& src_loc) const {
    return executeInContext(src_loc, glGetBufferParameteriv, id(), GL_BUFFER_STORAGE_FLAGS, dst.cast_reinterpret<Int>());
}

bool Buffer::getUsage(const Val<BufferUsage>& dst, const Val<const SrcLoc>& src_loc) const {
    return executeInContext(src_loc, glGetBufferParameteriv, id(), GL_BUFFER_USAGE, dst.cast_reinterpret<Int>());
}
...
bool Buffer::copySubDataTo(const Buffer& dst, const Val<const IntPtr>& read_offset,
                           ...
                           const Val<const SrcLoc>& src_loc) const {
    return executeInContext(src_loc, glCopyNamedBufferSubData, id(), dst.id(), read_offset, write_offset, size);
}

bool Buffer::getMapPointer(const Val<MapPtr>& dst, const Val<const SrcLoc>& src_loc) const {
...
bool Buffer::map(const Val<MapPtr>& dst, const Val<const BufferMapAccess>& access, const Val<const SrcLoc>& src_loc){
    return executeInContext(src_loc, &_glMapNamedBuffer, dst, id(), access.cast_reinterpret<const Enum>());
}
...
bool Buffer::mapRange(const Val<MapPtr>& dst, const Val<const IntPtr>& offset,
                      const Val<const SizeiPtr>& size, const Val<const BitField>& access, 
                      const Val<const SrcLoc>& src_loc){
    return executeInContext(src_loc, &_glMapNamedBufferRange, dst, id(), offset, size, access);
}
...
bool Buffer::unmap(const Val<bool>& dst, const Val<const SrcLoc>& src_loc){
    return executeInContext(src_loc, &_glUnmapNamedBuffer, dst, id());
}
