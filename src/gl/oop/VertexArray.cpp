...
std::shared_future<bool> VertexArray::setElementBuffer(const Buffer& buffer, const SrcLoc& loc){
    return _callMethod<CtxVertexArray, &CtxVertexArray::setElementBuffer>(buffer._getVop<CtxBuffer>(), loc);
}

std::shared_future<bool> VertexArray::setVertexBuffer(const Vop<gl::UInt>& binding, const Buffer& buffer,
                                                      const Vop<gl::IntPtr>& offset, const Vop<Sizei>& stride,
                                                      const SrcLoc& loc){
    return _callMethod<CtxVertexArray, &CtxVertexArray::setVertexBuffer>(binding, buffer._getVop<CtxBuffer>(),
                                                                         offset, stride, loc);
}