...
Texture::Texture(wptr<Context> ctx, const Vop<gl::TextureType> type, const SrcLoc& loc) : 
    Object(ctx, &make_sptr<gl::CtxTexture, const TextureType&, const SrcLoc&>, type, Vop<SrcLoc>(loc)){
}

std::shared_future<bool> Texture::buffer(const Vop<gl::TextureFormat> fmt, const Vop<Buffer> buffer, const SrcLoc& loc){
    static auto func = [](sptr<CtxTexture>* gl,
                          const sptr<const CtxBuffer>* buffer, const TextureFormat& fmt,
                          const SrcLoc& loc){
        auto raw_texture = gl->get();
        auto raw_buffer = buffer->get();
        raw_texture->buffer(fmt, *raw_buffer, loc);
    };
    return _execute(func, _getPtr<CtxTexture>(), getVopRef(buffer)._getPtr<CtxBuffer>(), fmt, loc);
}

std::shared_future<bool> Texture::bufferRange(const Vop<gl::TextureFormat> fmt, const Vop<Buffer> buffer,
                                              const Vop<gl::IntPtr> offset, const Vop<gl::SizeiPtr> size,
                                              const SrcLoc& loc){
    static auto func = [](sptr<CtxTexture>* gl,
                          const sptr<const CtxBuffer>* buffer, const TextureFormat& fmt,
                          const IntPtr& offset, const SizeiPtr& size,
                          const SrcLoc& loc){
        auto raw_texture = gl->get();
        auto raw_buffer = buffer->get();
        raw_texture->bufferRange(fmt, *raw_buffer, offset, size, loc);
    };
    return _execute(func, _getPtr<CtxTexture>(), getVopRef(buffer)._getPtr<CtxBuffer>(), fmt, offset, size, loc);
}
