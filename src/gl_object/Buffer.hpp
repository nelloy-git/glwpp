...
public:
    EXPORT Buffer(const std::shared_ptr<Context>& ctx, const SrcLoc& src_loc = SrcLoc{}) :
        ObjectHandle(ctx, 0, [](std::weak_ptr<Context> wctx, const GLuint* id, const SrcLoc& src_loc){
            if (auto ctx = wctx.lock()){
                ctx->addCallCustom(src_loc, [](Context& ctx, const GLuint* id_ptr){
                    ctx.gl.DeleteBuffers(1, id_ptr);
                }, Value<const GLuint*>(id));
            }
        }, SrcLoc(src_loc)){
        ctx->addCallGl<&GladGLContext::CreateBuffers>(src_loc.add(), Value<GLsizei>(1), getData());
    }
    ...
    template<Context::IsGlThread is_gl_thread = Context::IsGlThread::Unknown>
    EXPORT void copySubDataFrom(const Buffer& readBuffer, const ConstIntptr& readOffset, const ConstIntptr& writeOffset, const ConstSizeiptr& size, const SrcLoc& src_loc = SrcLoc{}){
    ...
    EXPORT void bindBase(const ConstEnum& target, const ConstUint& index, const SrcLoc& src_loc = SrcLoc{}){
        return _addCallGl<&GladGLContext::BindBuffer, is_gl_thread>(src_loc.add(), target, index, id());
    }
