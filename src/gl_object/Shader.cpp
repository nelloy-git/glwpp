...
void Shader::source(const GLstring& code, const SrcLoc& src_loc){
    _callGLCustom([](Gl& gl, const GLuint& id, const GLstring& code, const SrcLoc& src_loc){
...
void Shader::getParameteriv(const GLenum& pname, const GLintArr& params, const SrcLoc& src_loc){
    _callGL<&Gl::GetShaderiv>(id(), pname, params, src_loc);
}
...
GL::GLstring Shader::getInfoLog(const SrcLoc& src_loc){
    return _callGLCustom([](Gl& gl, const GLuint& id, const SrcLoc& src_loc){
        GLint len;
