...
public:
    CtxTexture(const TextureType& type, const SrcLoc& loc = SrcLoc::current());
    
    void buffer(const TextureFormat& fmt, const CtxBuffer& buffer, const SrcLoc& loc = SrcLoc::current());
    void bufferRange(const TextureFormat& fmt, const CtxBuffer& buffer,
                     const IntPtr& offset, const SizeiPtr& size, const SrcLoc& loc = SrcLoc::current());
    void image1D(const Sizei& levels, const TextureFormat& fmt,
                const Sizei& width, const SrcLoc& loc = SrcLoc::current());
    void image2D(const Sizei& levels, const TextureFormat& fmt,
                 const Sizei& width, const Sizei& height, const SrcLoc& loc = SrcLoc::current());
    void image3D(const Sizei& levels, const TextureFormat& fmt,
                 const Sizei& width, const Sizei& height, const Sizei& depth, const SrcLoc& loc = SrcLoc::current());
    ...
    void storageMultisample2D(const Sizei& samples, const TextureFormat& fmt,
                              ...
                              const Bool& fixed_locations, const SrcLoc& loc = SrcLoc::current());
    ...
    void setUnit(const UInt& index, const SrcLoc& loc = SrcLoc::current()) const;
    void generateMipMap(const SrcLoc& loc = SrcLoc::current());
    ...
    void setSubImage1D(const Int& level,
                       ...
                       const Data data, const SrcLoc& loc = SrcLoc::current());
    ...
    Wrap getWrapS(const SrcLoc& loc = SrcLoc::current()) const;
    void setWrapS(const Wrap& wrap, const SrcLoc& loc = SrcLoc::current());
    Wrap getWrapT(const SrcLoc& loc = SrcLoc::current()) const;
    void setWrapT(const Wrap& wrap, const SrcLoc& loc = SrcLoc::current());
    ...
    MinFilter getMinFilter(const SrcLoc& loc = SrcLoc::current()) const;
    void setMinFilter(const MinFilter& filter, const SrcLoc& loc = SrcLoc::current());
    ...
    MagFilter getMagFilter(const SrcLoc& loc = SrcLoc::current()) const;
    void setMagFilter(const MagFilter& filter, const SrcLoc& loc = SrcLoc::current());
