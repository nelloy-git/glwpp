...
public:
    ...
    std::shared_future<bool> buffer(const Vop<gl::TextureFormat>& fmt, const Buffer& buffer,
                                    const SrcLoc& loc = SrcLoc());
    std::shared_future<bool> bufferRange(const Vop<gl::TextureFormat>& fmt, const Buffer& buffer,
    ...
    std::shared_future<bool> setUnit(const Vop<gl::UInt>& index, const SrcLoc& loc = SrcLoc()) const;
    std::shared_future<bool> generateMipMap(const SrcLoc& loc = SrcLoc());
    ...
    std::shared_future<bool> getWrapS(Ptr<Wrap>& dst, const SrcLoc& loc = SrcLoc());
    std::shared_future<bool> setWrapS(const Vop<Wrap>& wrap, const SrcLoc& loc = SrcLoc());
    std::shared_future<bool> getWrapT(Ptr<Wrap>& dst, const SrcLoc& loc = SrcLoc());
    std::shared_future<bool> setWrapT(const Vop<Wrap>& wrap, const SrcLoc& loc = SrcLoc());

    using MinFilter = gl::CtxTexture::MinFilter;

    std::shared_future<bool> getMinFilter(Ptr<MinFilter>& dst, const SrcLoc& loc = SrcLoc());
    std::shared_future<bool> setMinFilter(const Vop<MinFilter>& filter, const SrcLoc& loc = SrcLoc());

    using MagFilter = gl::CtxTexture::MagFilter;

    std::shared_future<bool> getMagFilter(Ptr<MagFilter>& dst, const SrcLoc& loc = SrcLoc());
    std::shared_future<bool> setMagFilter(const Vop<MagFilter>& filter, const SrcLoc& loc = SrcLoc());
};
