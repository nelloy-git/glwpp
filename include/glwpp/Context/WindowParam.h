...
namespace glw::Context {

enum class WindowGLparam {
    BLEND,    // If enabled, blend the computed fragment color values with the values in the color buffers. See glBlendFunc.
    CLIP_DISTANCE_0, // If enabled, clip geometry against user-defined half space i.
    CLIP_DISTANCE_1, // If enabled, clip geometry against user-defined half space i.
    CLIP_DISTANCE_2, // If enabled, clip geometry against user-defined half space i.
    CLIP_DISTANCE_3, // If enabled, clip geometry against user-defined half space i.
    CLIP_DISTANCE_4, // If enabled, clip geometry against user-defined half space i.
    CLIP_DISTANCE_5, // If enabled, clip geometry against user-defined half space i.
    CLIP_DISTANCE_6, // If enabled, clip geometry against user-defined half space i.
    CLIP_DISTANCE_7, // If enabled, clip geometry against user-defined half space i.
    CLIP_DISTANCE_NV, // If enabled, clip geometry against user-defined half space i.
    ...
    DEBUG_OUTPUT, // If enabled, debug messages are produced by a debug context. When disabled, the debug message log is silenced. Note that in a non-debug context, very few, if any messages might be produced, even when GL_DEBUG_OUTPUT is enabled.
    ...
    DEPTH_CLAMP,  // If enabled, the −wc≤zc≤wc plane equation is ignored by view volume clipping (effectively, there is no near or far plane clipping). See glDepthRange.
    ...
    POLYGON_OFFSET_FILL,  // If enabled, and if the polygon is rendered in GL_FILL mode, an offset is added to depth values of a polygon's fragments before the depth comparison is performed. See glPolygonOffset.
    POLYGON_OFFSET_LINE,  // If enabled, and if the polygon is rendered in GL_LINE mode, an offset is added to depth values of a polygon's fragments before the depth comparison is performed. See glPolygonOffset.
    POLYGON_OFFSET_POINT, // If enabled, an offset is added to depth values of a polygon's fragments before the depth comparison is performed, if the polygon is rendered in GL_POINT mode. See glPolygonOffset.
    ...
    PRIMITIVE_RESTART,    // Enables primitive restarting. If enabled, any one of the draw commands which transfers a set of generic attribute array elements to the GL will restart the primitive when the index of the vertex is equal to the primitive restart index. See glPrimitiveRestartIndex.
    ...
    RASTERIZER_DISCARD,   // If enabled, primitives are discarded after the optional transform feedback stage, but before rasterization. Furthermore, when enabled, glClear, glClearBufferData, glClearBufferSubData, glClearTexImage, and glClearTexSubImage are ignored.
    ...
    SAMPLE_COVERAGE,  // If enabled, the fragment's coverage is ANDed with the temporary coverage value. If GL_SAMPLE_COVERAGE_INVERT is set to GL_TRUE, invert the coverage value. See glSampleCoverage.
    // SAMPLE_SHADING,   // If enabled, the active fragment shader is run once for each covered sample, or at fraction of this rate as determined by the current value of GL_MIN_SAMPLE_SHADING_VALUE. See glMinSampleShading.
    SAMPLE_MASK,  // If enabled, the sample coverage mask generated for a fragment during rasterization will be ANDed with the value of GL_SAMPLE_MASK_VALUE before shading occurs. See glSampleMaski.
